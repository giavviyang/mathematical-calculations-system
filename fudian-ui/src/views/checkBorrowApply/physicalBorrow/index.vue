    ...
    >
      <el-tab-pane label="待审批" name="pendapproval">
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row  v-hasPermi="['checkBorrowApply:physicalBorrow:search']"  :gutter="10" ref="searchRow1" style="margin:0 0 0 0;padding-left: 0px">
            <el-form
              v-if="showBorrowing"
              class="search-form"
          ...
          <div class="btn">
            <el-button type="primary" size="mini" icon="el-icon-plus" v-hasPermi="['checkBorrowApply:physicalBorrow:startApply']"   @click="handleAddBorrowing">发起申请</el-button>
            <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['checkBorrowApply:physicalBorrow:export']"  @click="handleExportBorrowing">导出</el-button>
          </div>
          ...
          >
            ...
            <el-table-column  label="操作" align="center" width="100" fixed="right">
                  <template slot-scope="scope">
                    <el-button type="text" size="small" v-hasPermi="['checkBorrowApply:physicalBorrow:details']"  @click="electronicFeedback(scope.row)">借看详情</el-button>
                  </template>
                </el-table-column>
          </el-table>
      ...
      <el-tab-pane label="已通过" name="passed">
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row :gutter="10"  ref="searchRow2"  style="margin:0 0 0 0;padding-left: 0px">
            <el-form
              v-hasPermi="['checkBorrowApply:physicalBorrow:search']"
              v-if="showBorrowing"
              class="search-form"
            ...
            >
              <el-form-item label="申请借阅时间起"   prop="startTime">
                <el-date-picker  size="mini"  v-model="borrowingParams1.startTime"  type="date" placeholder="请选择时间"  style="width: 150px" />
              </el-form-item>
              <el-form-item label="申请借阅时间至"   prop="endTime" >
          ...
          <div class="btn">
            <el-button type="primary" icon="el-icon-receiving" size="mini" v-hasPermi="['checkBorrowApply:physicalBorrow:renewal']"   @click="renewalOfMaturity">到期续借</el-button>
            <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['checkBorrowApply:physicalBorrow:export']"   @click="handleExportBorrowing">导出</el-button>
          </div>
        ...
        <!--  到期续借  -->
    <el-dialog title="到期续借"  width="28%" class=" dialog-style dialog-basic" :visible.sync="viewDetalsVasable1">
      ...
      <el-tab-pane label="未通过" name="notpass">
        ...
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          ...
          <div class="btn">
            <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['checkBorrowApply:physicalBorrow:export']" @click="handleExportBorrowing">导出
            </el-button>
          </div>
      ...
      <el-col  :span="24" :xs="24" style="margin: 0;padding: 0;border: 1px solid #cccccc;height: calc(100vh - 86px);overflow: auto;">
        <el-row  style="padding-left: 10%;padding-right: 10%">
          <el-col :span="24" style="width:100%">
            ...
            <el-form :rules="rules"  ref="addForm" :model="addForm" label-width="150px" style="width:90%;padding: 20px 0 0 0;margin: 0 auto;padding-left: 3%;padding-right: 3%">
          ...
          <el-col :span="24" >
            ...
            <el-col :span="24" style="padding-left: 2%;padding-right: 2%">
              <el-table height="222" size="mini" empty-text=" " tooltip-effect="light"  ref="searchTableData1" class="table-style" :data="searchTableData" border @cell-dblclick="deptClick" @selection-change="gridSelectionChange" :header-cell-style="headClass" @row-click="handleGridChange" :stripe="true">
          ...
          <el-table
            ...
            >
              ...
              <el-table-column show-overflow-tooltip align="center" v-if="true" label="借档内容" prop="e01z111a" />
...
export default {
  data() {
    return {
      loading:true,
      testShow:false,
      pdfUrl:"/fileStorage/getTestFileBytes",
      //普通参数
      tabsShow:true,
      formShow:false,
      showBorrowing:true,
      ...
      //干部搜索用
      checkA01Col:[
        { isCol: true,style:"width: 1%",type:"selection",style:"width:40px"},
        { label: '序号', isCol: true,type:"index",style:"width: 1%"},
        { label: '干部姓名', prop: 'a0101', isCol: true},
        // { label: '干部编号', prop: 'number', isCol: true},
        { label: '所属部门', prop: 'deptName', isCol: true},
        { label: '性别', prop: 'a0104', isCol: true,formatter:this.getFullName},
        { label: '出生日期', prop: 'a0107', isCol: true,formatter:this.getFullName},
        { label: '民族', prop: 'a0117', isCol: true,formatter:this.getFullName},
        { label: '政治面貌', prop: 'a0141', isCol: true,formatter:this.getFullName},
      ],
      //部门搜索用
      checkDeptCol:[
        { label: '序号', isCol: true,type:"index",style:"width: 1%"},
        { label: '部门名称', prop: 'deptName', isCol: true},
        { label: '上级部门', prop: 'parentId', isCol: true},
      ],
      rules:{
        e01z801:[
          { required: true, message: '审批人必须填写',trigger: "blur" },
        ...
        flag:[
          { required: true, message: '审批意见必须填写' ,trigger: "blur"},
          { min: 1, max: 255, message: '长度在 2 到 255 个字符',trigger: "blur" }
        ],
        userName: [
  ...
  methods: {
    ...
    archivesaFunction(all) {
      if(this.applyStates=="0"||all){
        ...
        getEntityArchiveAApplyRecord(pageSizeParam).then(response =>{
    ...
    handleExportBorrowing(){
      ...
      // console.log(s);
      // s = encodeURI(s);
      // downLoad("/export/excel/export?status=5&exportData=" + s)
    },
    headClass() {
