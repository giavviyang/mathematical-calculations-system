<template>
  <div class="app-archive-check-paramenter">
    <el-row style="height:100%" v-if="tabsShow">
        ...
        <el-row ref="searchRow" :gutter="10">
          <el-form
            ...
            <el-form-item @keyup.enter.native="onEnterSearch" label="申请人姓名"  prop="userName" style="margin-top: 10px;margin-bottom: 10px">
              <el-input v-model="queryParams.userName" placeholder="请输入查阅人姓名" clearable size="mini" style="width: 150px" />
            </el-form-item>
            <el-form-item label="申请查阅时间起" prop="beginTime" style="margin-top: 10px;margin-bottom: 10px">
              <el-date-picker v-model="queryParams.beginTime" value-format="yyyy-MM-dd" type="date" placeholder="请选择查阅起始时间" size="mini" style="width:150px"/>
            </el-form-item>
            <el-form-item label="申请查阅时间至" prop="endTime" style="margin-top: 10px;margin-bottom: 10px">
              <el-date-picker v-model="queryParams.endTime" value-format="yyyy-MM-dd" type="date" placeholder="请选择查阅截止时间" size="mini" style="width:150px"/>
            </el-form-item>
            <el-form-item style="margin-top: 10px;margin-bottom: 10px">
              <el-button type="primary" icon="el-icon-search" size="mini" @click="queryArchive">搜索</el-button>
              <!-- <el-button icon="el-icon-refresh" size="mini" @click="reset">重置</el-button> -->
            </el-form-item>
          </el-form>
          <div class="btn">
            <el-button type="primary" icon="el-icon-plus" size="mini" v-hasPermi="['parameter:entityCheck:add']" @click="archiveCheck(1)">新增</el-button>
            <el-button type="primary" icon="el-icon-edit" size="mini" v-hasPermi="['parameter:entityCheck:edit']" @click="archiveCheck(2)">编辑</el-button>
            <el-button type="primary" icon="el-icon-delete" size="mini" v-hasPermi="['parameter:entityCheck:delete']" @click="deleteArchive">删除</el-button>
            <el-button type="primary" icon="el-icon-download" size="mini" v-hasPermi="['parameter:entityCheck:import']" @click="handleExportArchive">导出</el-button>
            <!--
            <div class="btn-search">
              <el-button type="primary" icon="el-icon-search" size="mini" circle @click="function(){showArchive = !showArchive;tableHeight = showArchive?'73%':'80%'}" />
              <el-dropdown style="float: right; margin-left: 5px;">
                ...
                <el-dropdown-menu style="padding: 10px">
                  <el-checkbox-group v-for="(item, index1) in EntityColumn" :key="index1" v-model="item.isCol" size="mini">
      ...
      <el-dialog
        ...
        >
        <div class="btn">
          <el-button type="primary" icon="el-icon-download" size="mini" @click="handleExportArchive">导出</el-button>
        </div>
        <el-table
          size="mini"
          :data="detailsData"
          ...
          <el-table-column prop="startTime" label="申请查阅起始日期" align="center"   />
          <el-table-column prop="endTime" label="申请查阅截止日期至" align="center"   />
          ...
          <el-form :rules="rules" ref="addForm" :model="addForm" label-width="135px" style="margin-top:25px;padding-left: 2%;padding-right: 2%">
            ...
            <el-col :span="8">
              <el-form-item label="申请查阅起始日期" prop="startTime">
                <el-date-picker size="mini" v-model="addForm.startTime"   value-format="yyyy-MM-dd"    type="date" placeholder="选择申请查阅起始日期" style="width: 100%;" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item  label="申请查阅截止日期" style="text-align:right;" prop="endTime">
                <el-date-picker size="mini" v-model="addForm.endTime"   value-format="yyyy-MM-dd"    type="date" placeholder="选择申请查阅截止日期" style="width: 100%;" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
...
<!--                <input  type="file" multiple="multiple" ref="imageFile" accept="image/png, image/jpeg" />-->
<!--              </el-form-item>-->
              <el-form-item  label="添加附件" prop='sysFile'>
...
<!--          <el-form  ref="addForm2" :model="addForm" label-width="130px" style="padding-left: 2%;padding-right: 2%">-->
<!--            -->
<!--          </el-form>-->
<!--        </el-col>-->
        ...
        <el-col :span="24" >
          ...
          <el-col :span="24" style="padding-left: 2%;padding-right: 2%">
            <el-table
              ...
              >
              <el-table-column type="selection" align="center" width="45" key="1" v-if="dpbutton"/>
...
<!--              <el-table-column prop="number" v-if="isa01 == 2" key="3" label="干部编号" align="center" />-->
        ...
        <el-table
          ...
          >
          <el-table-column style="width: 1%" type="index" align="center" label="序号" />
          <el-table-column align="center" v-if="true" label="干部姓名" prop="a0101" />
<!--          <el-table-column align="center" v-if="true" label="干部编号" prop="number" />-->
          ...
          <el-table-column align="center" v-if="true" label="查档内容" prop="e01z111a" />
...
<script>
import { queryDictionaryByType} from '@/api/usual/parameter/check-borrow-paramenter.js'
import { queryA01List } from '@/api/usual/transfer'
import { queryDictionaryByTypes,queryLoginUser} from '@/api/usual/receive'
import {queryE0z8s, queryE01z8LedgerList,queryE01z8A01s,queryYear,updateByEntityAndList,queryE01z8RelaLedgerList,deleteE01z8Ledger,deleteE01z8RelaLedger,getFileName} from "@/api/parameter/entity"
...
export default {
  ...
  data() {
    return {
      ...
      EntityColumn: [
        ...
        { label: '申请查阅截止日期', prop: 'endTime', isCol: true ,width:"150px", formatter:this.getFullName},
  ...
  methods: {
    ...
    archivesaFunction() {
      ...
      queryE0z8s(params).then(response=>{
    ...
    queryArchive() {
      const _this=this;
      searchArchivesCheckRecords(_this.queryParams)
        .then(response => {
          if (response.code === 200) {
    ...
    viewDetals(val){
      ...
      queryE01z8RelaLedgerList({e01z8Id:val.id}).then(response=>{
    ...
    typeClick() {
      ...
      for (let i = 0; i < gridDataList.length; i++) {
        ...
        for(let n in this.queryData){
          if(this.queryData[n].a01Id==gridDataList[i].id){
            a01s.push(gridDataList[i]);
            flag=false;
          }
        }
        if(flag){
    ...
    getFullName(row,col){
      // 时间
      if( col.property=="startTime"||col.property=="endTime"||col.property=="approvTime"||col.property=="creatersTime"||col.property=="a0107"){
        if(row[col.property]!=null){
          return this.moment(row[col.property]).format("YYYY-MM-DD")
        }
      }
      // 精确到时分秒的时间转换
      /*if(col.property=="endTime"){
        if(row[col.property]!=null){
    ...
    onGrid(addForm) {
      ...
      this.$refs[addForm].validate((valid) => {
        ...
        if (valid) {
          ...
          console.log(this.addForm2.id)
          if(this.addForm2.id!=""&&this.addForm2.id!=null){
            //修改方法
            updateByEntityAndList(testFormData).then(response=>{
              if(response.code==200){
                prompt('保存',0)
                this.formShow=false;
                this.tabsShow=true;
    ...
    archiveCheck(val){
      if(val == 1){
        ...
        getFileName({e01z8Id:this.checkboxDatas[0].e01z8Id}).then(res =>{
          if(res.code=='200'){
        ...
        queryE01z8A01s(this.checkboxDatas[0].e01z8Id).then(response=>{
    ...
    deleteRow(index, rows,row) {
      this.$confirm('此操作将永久删除所选信息, 是否继续?', '提示', {
        ...
        .then(() => {
          deleteE01z8RelaLedger(row.id).then(response=>{
            if(response.code == 200){
              rows.splice(index, 1)
            }
          })
    ...
    deleteArchive(value) {
      ...
      this.$confirm('此操作将永久删除所选信息, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          deleteE01z8RelaLedger(ids.join(",")).then(response=>{
            this.$message({
              message: '删除成功',
              type: 'success'
            });
    ...
    /**
    //删除实体档案台账
    handleDeleteArchive(value) {
      const _this = this;
      let param = {};
      param.ids = this.gridCheck.toString();
      param.type = _this.archivesType;
      if (this.gridCheck == 0) {
        this.$message({
          showClose: true,
          message: '请选择您要删除的内容',
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          .then(() => {
            deleteArchivesRecords(param)
              .then(function (response) {
                _this.$message({
                  type: 'success',
                  message: '删除实体档案台账!'
                })
                //_this.archivesaFunction(params);
              })
        ...
        this.$confirm('是否导出搜索后的台账数据?', "提示", {
          ...
          queryE0z8s(params).then(response=>{
            ...
            let data = {
              ...
              "tableTitle":"实体档案查阅台账表",
              "sheetTitle":"实体档案查阅台账表",
              "downLoadName":"实体档案查阅台账表"
            }
