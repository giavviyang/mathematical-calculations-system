...
<!--        id="treescroll"-->
      ...
      <el-col :span="20" style="height: calc(100vh - 86px)" :style="rightStyleName">
          <el-row ref="searchRow" v-hasPermi="['usually:daily:search']">
            <el-form :inline="true" ref="formSearch" label-width="100px" :model="formSearch" class="search-form">
              ...
              <el-form-item>
                ...
                <el-button type="primary" v-hasPermi="['usually:daily:seniorSearch']" size="mini" icon="el-icon-zoom-in" @click="searchDialog()">高级检索</el-button>
              </el-form-item>
            </el-form>
          </el-row>
        <div class="btn">
          <el-button type="primary" v-hasPermi="['usually:daily:add']" size="mini" icon="el-icon-plus" @click="addRecord">新增</el-button>
          <el-button type="primary" v-hasPermi="['usually:daily:delete']" size="mini" icon="el-icon-delete" @click="onDelete">删除</el-button>
          <!--<el-button type="primary" size="mini" icon="el-icon-sort" @click="sort">排序</el-button>-->
          <el-button type="primary" v-hasPermi="['usually:daily:adjustDepartment']" size="mini" v-model="showdaliog" icon="el-icon-set-up" @click="depementChange">
            部门调整
          </el-button>
          <el-button type="primary" v-hasPermi="['usually:daily:adjustStatus']" size="mini" v-model="showdaliog" icon="el-icon-set-up" @click="stateChange">状态调整
          </el-button>
          <!-- <el-button type="primary" v-hasPermi="['usually:daily:checkArchives']" size="mini" icon="el-icon-folder-add" @click="lack()">档案查缺</el-button> -->
          <el-button type="primary" v-hasPermi="['usually:daily:import']" size="mini" icon="el-icon-download" @click="onReturn">导出</el-button>
        </div>
        <el-table
          v-loading="loading"
          ref="tableRefs1"
          ...
          <el-table-column label="出生日期" prop="a0107" :formatter="conversionTime" show-overflow-tooltip width="100px"
                           align="center"></el-table-column>
          <el-table-column label="入党日期" prop="a0144"  show-overflow-tooltip width="100px"
                           align="center"></el-table-column>
          <el-table-column label="参加工作时间" prop="a0134" show-overflow-tooltip width="130px"
                           align="center"></el-table-column>
         ...
         <!-- <el-table-column label="编制类别" :formatter="tableFormatter" prop="data4" show-overflow-tooltip
          ...
          <el-table-column fixed="right" label="操作" align="center" width="150px">
            <template slot-scope="scope">
              <el-button type="text" v-hasPermi="['usually:daily:information']" @click="message(scope.row)" size="mini">基本信息</el-button>
              <router-link  v-hasPermi="['usually:daily:catalog']" :to="{path:'/usually/catalog/'+ scope.row.a0101 + '/' + scope.row.id}" class="link-type">
                目录维护
              </router-link>
        ...
        <div class="page" style="margin-top: 5px;">
          <el-pagination style="width: 100%;" @size-change="handleSizeChange" @current-change="handleCurrentChange"
                         :current-page="this.formSearch.pageNum" :page-sizes="[20, 50, 100, 200]" :page-size="20"
    ...
    :visible.sync="dialogVisible1"
      ...
      <el-form ref="form" :rules="formRules"  :inline="true" :model="form" label-position="right" >
        ...
        <el-form-item label="干部姓名" label-width="110px" prop="a0101" size="mini">
          <el-input v-model="form.a0101" @blur="loseBlur" placeholder="请输入干部姓名" clearable class="inputW"/>
        </el-form-item>
<!--        <el-form-item label="干部编号" label-width="110px" prop="number" size="mini">
        ...
        <el-form-item label="姓名简拼" label-width="110px" size="mini" prop="a0102">
          <el-input v-model="form.a0102" placeholder="请输入姓名简拼" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="性别" label-width="110px" size="mini" prop="a0104">
          <el-select v-model="form.a0104" placeholder="请选择性别" clearable class="inputW">
        ...
        <el-form-item label="民族" label-width="110px" prop="a0117" size="mini">
          <el-select v-model="form.a0117" placeholder="请选择民族" clearable class="inputW">
        ...
        <el-form-item label="部门" label-width="110px" size="mini">
          <el-select v-model="form.deptId" placeholder="请选择部门" clearable class="inputW" ref="selectReport">
        ...
        <el-form-item label="出生日期" label-width="110px" prop="a0107" size="mini">
          <el-date-picker value-format="yyyy-MM-dd" v-model="form.a0107" type="date" placeholder="请选择出生日期" clearable class="inputW"/>
        </el-form-item>
        <el-form-item label="入党日期" label-width="110px" prop="a0144" size="mini">
          <el-input v-model="form.a0144" maxlength="10" placeholder="请输入入党时间" clearable class="inputW" />
        </el-form-item>
        <el-form-item label="参加工作日期" label-width="110px" prop="a0134" size="mini">
          <el-input v-model="form.a0134" maxlength="10" placeholder="请输入参加工作日期" clearable class="inputW" />
        </el-form-item>
        <el-form-item label="学历" label-width="110px" prop="education" size="mini">
          <el-select v-model="form.education" placeholder="请选择学历" clearable class="inputW">
            <el-option v-for="item in education" :key="item.dictCode" :label="item.fullName" :value="item.dictCode"/>

          </el-select>
        </el-form-item>
        <el-form-item label="人员现状" label-width="110px" prop="state" size="mini">
          <el-select v-model="form.state" placeholder="请选择人员现状" clearable class="inputW">
        ...
        <!-- <el-form-item label="编制类别" label-width="110px" prop="data4" size="mini">
          <el-select v-model="form.data4" placeholder="请选择编制类别" clearable class="inputW">
            <el-option v-for="item in data4" :label="item.fullName" :value="item.dictCode"/>
          </el-select>
        </el-form-item> -->
        <el-form-item label="个人身份" label-width="110px" prop="a0151" size="mini">
          <el-select v-model="form.a0151" placeholder="请选择个人身份" clearable class="inputW">
            <el-option v-for="item in a0151s" :key="item.dictCode" :label="item.fullName" :value="item.dictCode"/>
          </el-select>
        </el-form-item>
        <el-form-item label="职务" label-width="110px"  prop="duty" size="mini">
          <el-select v-model="form.duty" filterable  placeholder="请选择职务" clearable class="inputW">
            <el-option v-for="item in dutys" :key="item.dictCode" :label="item.fullName" :value="item.dictCode" />
          </el-select>
        </el-form-item>
        <div class="form-supplement" style="font-weight: bold">补充信息</div>
        <el-form-item label="籍贯名称" label-width="200px" prop="a0111a" size="mini">
          <el-input v-model="form.a0111a" placeholder="请输入籍贯名称" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="出生地代码" label-width="200px" prop="a0114b" size="mini">
          <el-input v-model="form.a0114b" placeholder="请输入出生地代码" clearable class="inputW"></el-input>
        ...
        <el-form-item label="健康状况描述" label-width="200px" prop="a0128" size="mini">
          <el-input v-model="form.a0128" placeholder="请输入健康状况描述" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="婚姻状况" label-width="200px" prop="a0131" size="mini">
          <el-select v-model="form.a0131" placeholder="请选择婚姻状况" clearable class="inputW">
        ...
        <el-form-item label="健康状况" label-width="200px" prop="a0127" size="mini">
          <el-select v-model="form.a0127" placeholder="请选择健康状况" clearable class="inputW">
        ...
        <el-form-item label="工龄计算正值" label-width="200px" prop="a0137" size="mini">
          <el-input  v-model.number="form.a0137" placeholder="请输入工龄计算正值" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="政治面貌" label-width="200px" prop="a0141" size="mini">
          <el-select v-model="form.a0141" placeholder="请选择政治面貌" clearable class="inputW">
        ...
        <el-form-item label="现身份起始日期" label-width="200px" prop="a0154" size="mini" type="date">
          <el-date-picker type="date"  value-format="yyyy-MM-dd" v-model="form.a0154" placeholder="请选择现身份起始日期" clearable class="inputW"></el-date-picker>
        </el-form-item>
        <el-form-item label="户口性质" label-width="200px" prop="a0177" size="mini">
          <el-select v-model="form.a0177" placeholder="请选择户口性质" clearable class="inputW">
        ...
        <el-form-item label="户籍所在地" label-width="200px" prop="a0181b" size="mini">
          <el-input v-model="form.a0181b" placeholder="请输入户籍所在地" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="有效证件类型" label-width="200px" prop="a0183" size="mini">
          <el-select v-model="form.a0183" placeholder="请选择有效证件类型" clearable class="inputW">
        ...
        <el-form-item label="证件号" label-width="200px" prop="a0184" size="mini">
          <el-input v-model="form.a0184" placeholder="请输入证件号" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="籍贯代码" label-width="200px" prop="a0111b" size="mini">
          <el-input v-model="form.a0111b" placeholder="请输入出生地代码" clearable class="inputW"></el-input>
        ...
        <el-form-item label="成长地" label-width="200px" prop="a0115a" size="mini">
          <el-input v-model="form.a0115a" placeholder="请输入成长地" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="成长地代码" label-width="200px" prop="a0115b" size="mini">
          <el-input v-model="form.a0115b" placeholder="请输入成长地代码" clearable class="inputW"></el-input>
        ...
        <el-form-item label="户籍所在地代码" label-width="200px" prop="a0181b" size="mini">
          <el-input v-model="form.a0181a" placeholder="请输入户籍所在地代码" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="出生地名称" label-width="200px" prop="a0114a" size="mini">
          <el-input v-model="form.a0114a" placeholder="请输入出生地名称" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="爱好" label-width="200px" prop="a0187b" size="mini">
          <el-input v-model="form.a0187b" placeholder="请输入爱好" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="专长" label-width="200px" prop="a0187a" size="mini">
          <el-input v-model="form.a0187a" placeholder="请输入专长" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="人事关系所在单位名称" label-width="200px" prop="a0157a" size="mini">
          <el-input v-model="form.a0157a" placeholder="请输入人事关系所在单位名称" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="人事关系所在单位代码" label-width="200px" prop="a0157b" size="mini">
          <el-input v-model="form.a0157b" placeholder="请输入人事关系所在单位代码" clearable class="inputW"></el-input>
        ...
        <el-form-item label="人事关系所在单位所在政区" label-width="200px" prop="a0161" size="mini">
          ...
          <el-input v-model="form.a0161" placeholder="请选择人事关系所在单位所在政区" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="人事关系所在单位隶属关系" label-width="200px" prop="a0164" size="mini">
          <el-select v-model="form.a0164" placeholder="请选择人事关系所在单位隶属关系" clearable class="inputW">
        ...
        <el-form-item label="人事关系所在单位级别" label-width="200px" prop="a0197" size="mini">
          <el-select v-model="form.a0197" placeholder="请选择人事关系所在单位级别" clearable class="inputW">
        ...
        <el-form-item label="人事关系所在单位性质类别" label-width="200px" prop="a0171" size="mini">
          <el-select v-model="form.a0171" placeholder="请选择人事关系所在单位性质类别" clearable class="inputW">
        ...
        <el-form-item label="人事关系所在单位所属行业" label-width="200px" prop="a0174" size="mini">
          <el-input v-model="form.a0174" placeholder="请输入人事关系所在单位所属行业" clearable class="inputW"></el-input>
        </el-form-item>
        <el-form-item label="声像信息标识" label-width="200px" prop="a0191" size="mini">
          <el-select v-model="form.a0191" placeholder="请选择声像信息标识" clearable class="inputW">
