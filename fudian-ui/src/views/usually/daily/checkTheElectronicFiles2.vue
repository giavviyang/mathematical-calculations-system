     ...
     <el-row>
       ...
       <el-col :span="24" style="height: calc(100vh - 84px);">
         ...
         <div class="btn" v-if="isBrowse">
           <el-button :disabled="isBut" type="primary" size="mini" v-if="upBtnHidden" icon="el-icon-printer" class="top-btn" @click="sendMessage(2)">导入</el-button>
           <el-button :disabled="isBut" style="margin: 5px;" v-if="upBtnHidden" type="primary" size="mini" icon="el-icon-folder-add" @click="sendMessage(1)">扫描</el-button>
           ...
           <el-button style="margin: 5px;" v-if="upBtnHidden" type="primary" size="mini" icon="el-icon-s-grid" @click="TheSorting">排序</el-button>
...
<!--            <el-button style="margin: 5px;" v-if="browseBtnHidden" type="primary" icon="el-icon-caret-left" @click="OnThePicture" size="mini">上页</el-button>
            ...
            <el-button v-if="false" style="margin: 5px;" type="primary" size="mini" icon="el-icon-upload" @click="browseMode">{{modelText}}</el-button>
         </div>
         <el-col :span="24" >
           ...
           <el-col :span="24" v-if="leftShow">
            ...
            <el-col :span="24" v-if="rightShow">
               ...
               <el-col :span="21" ref="rightBrowse" id="rightBrowse" style="position: relative;height: calc(100vh - 170px);overflow: auto;">
                 <div style="position: relative;height: calc(100vh - 170px);overflow: auto;">
                  ...
                  <el-col :span="2" class="leftRightBt">
                    <div class="el-image-viewer__btn el-image-viewer__prev prevPromptF" @click="onAFile"><span class="promptOnF">上一份</span><i class="el-icon-d-arrow-left" /></div>
                    <div style="margin-left: 120px;" class="el-image-viewer__btn el-image-viewer__prev prevPromptY" @click="OnThePicture"><span class="promptOnY">上一页</span><i class="el-icon-arrow-left" /></div>
                  </el-col>
                  <el-col :span="2" class="RightleftBt">
                     <div style="margin-right: 120px;" class="el-image-viewer__btn el-image-viewer__next nextPicture" @click="nextPicture"> <span class="promptFNextY">下一页</span><i class="el-icon-arrow-right" /></div>
                     <div class="el-image-viewer__btn el-image-viewer__next nextpromptF" @click="underAFile"><span class="promptFNextF">下一份</span><i class="el-icon-d-arrow-right" /></div>
                  </el-col>
               </el-col>
            </el-col>
             ...
             <el-form :rules="rules" :inline="true"  :model="formElectronicFile">
              <el-form-item label="材料序号" label-width="80px">
               ...
               <el-form-item label="材料名称" prop="e01z111a" label-width="80px">
                 <el-input style="width: 150px;" v-model="formElectronicFile.e01z111a" :disabled="isDisabled"></el-input>
               </el-form-item>
               </el-form-item>
               ...
               <el-form-item>
                   <el-button size="mini" @click="fileEditor">{{fileBt}}</el-button>
               </el-form-item>
             </el-form>
             <el-dialog
 ...
 <script>
   ...
   import {queryId,upDataParams,printImgPdfBy,queryImgInfoByE01z1Id,download,downloadFile,updateE01z1} from '@/api/usually/daily/materials'
   // 获取当前网站端口
 ...
 export default {
   ...
   mounted() {
      ...
      let rightHealth = document.getElementById("app-wait");
 ...
 //在外部vue的window上添加postMessage的监听，并且绑定处理函数handleMessage
      ...
      if(material.isShow === true){
        this.isBrowse = true
        this.rightBoxStyle = 'rightBoxStyle'
        let _this = this;
         setTimeout(()=>{  //设置延迟执行
        ...
        this.isBrowse = false
        this.rightBoxStyle = 'rightBoxStyle1'
      }
   },
   methods: {
     //  给 iframe 传参
    sendMessage ( num ) {
          ...
          if( response.code === 200 ){
            if( response.data.length === 0 ){
                ...
                for (let i = 0; i < response.data.length; i++) {
                  this.urlAll.push({
                     isActive:true,
                     src:baseUrl + "/fileStorage/downImage?fileId="+response.data[i].fileSource+"&compress="+isType+"",
     ...
     browseMode(){
      ...
      if( this.rightShow === true ){
        ...
        if( this.urlAll.length >= 1 ){
          ...
          if( this.urlAll[this.leftNum].type === 1 ){
            this.rightSrc = baseUrl + "/fileStorage/downImage?fileId="+this.urlAll[this.leftNum].fileSource+"&compress=false"
          }else{
            this.rightSrc = this.urlAll[this.leftNum].src
          }
      ...
      if( this.leftShow === true ){
        ...
        if( this.urlAll.length >= 1 ){
          if( this.urlAll[this.leftNum].type === 1 ){
            this.url = baseUrl + "/fileStorage/downImage?fileId="+this.urlAll[this.leftNum].fileSource+"&compress=false"
          }else{
        ...
        if(!this.leftShow){
          ...
          if( this.urlAll.length === 0 ){
            ...
            if (this.urlAll[this.currentPages].type === 1) {
              this.url = baseUrl + "/fileStorage/downImage?fileId="+this.urlAll[this.currentPages].fileSource+"&compress=false";
            }else{
     ...
     leftPicClick(num){
      ...
      // this.urlAll[this.oldNum].isActive = true;
       if (this.urlAll[num].type === 1) {
         this.rightSrc = baseUrl + "/fileStorage/downImage?fileId="+this.urlAll[num].fileSource+"&compress=false";
       }else{
       ...
       if( num === '1' ){
        ...
        .then(response => {
            if( response.code === 200 ){
              ...
              let pdfUrl =  baseUrl + "/common/printpdf?id="+response.data // 调取接口返回文件流
       ...
       if( this.urlAll.length < 1 ){
        ...
        if( this.urlAll.length < 1 ){
          ...
          if( this.leftNum == 0 ){
            ...
            this.deleteList.push(this.urlAll.splice(this.leftNum,1)[0]);
            this.rightSrc = baseUrl + "/fileStorage/downImage?fileId="+this.urlAll[this.leftNum-1].fileSource+"&compress=false";
