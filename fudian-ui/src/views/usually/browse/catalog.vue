  ...
  <div class="app-receive">
    ...
    <el-col v-loading="loadding" :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px);overflow: auto;">
        <el-row :gutter="10"  ref="searchRow" style="margin:0 0 10px 0;padding-left: 0px">
            <el-col :span="1.5">
              <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['use:browse:catalogexport']" @click="makeExcel">导出excel</el-button>
            </el-col>

            <el-col :span="1.5">
              <el-button type="primary" size="mini" icon="el-icon-s-cooperation"
                         v-hasPermi="['use:browse:file']" @click="manageEfile">电子档案浏览</el-button>
            </el-col>
    ...
    <!-- 材料分类调整 -->
    <el-dialog title="材料调整到其他分类" class="dialog-style" :close-on-click-modal="false" :visible.sync="dialogVisible2"
               width="40%">
...
<script>
  ...
  import { downLoad,requestDownLoad,requestExcel } from '@/downLoad/filedownload'
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 录入材料 显示
        entryShow: true,
        testA01Id:this.$route.params.id,
    ...
    methods: {
      ...
      makeExcel(){
        ...
        let insertRecords = XEUtils.filterTree(this.queryData, item => item.isNew, this.treeConfig)
        let removeRecords = this.removeTreeData
        console.info(insertRecords.length>0)
        console.info(removeRecords.length>0)
        console.info(this.editDatas.length>0)
        console.info(!this.moveArchive)
        console.info(this.isAllRemove)
        ...
        requestExcel('/e01z1/getE01z1Excel?a01Id=' + this.testA01Id,(response)=>{
          if(response.code == 200){
            return true;
          }
        })
        //download("/e01z1/getE01z1Excel?a01Id="+this.testA01Id);
        // window.location.href="/e01z1/getE01z1Excel?a01Id="+this.testA01Id
        // requestDownLoad(downLoad('/e01z1/getE01z1Excel',{a01Id:this.testA01Id}),callback);
      },
      editClosed({row}){
        //如果修改过则放入修改条目集合中
        ...
        xTree.validate(row, (errMap) => {
          if (errMap) {
            ...
            if(returnParam.a01Id==null||returnParam.a01Id==''){
              returnParam.a01Id=this.testA01Id;
            }
      ...
      operationRightButtonRemove(row){
          ...
          this.$confirm('此操作将移除所选档案材料, 是否继续?', '提示', {
      ...
      saveE01z1s(){
        ...
        xTree.validate(tableListData, (errMap) => {
          if (errMap) {
            let params={
              e01z1s:this.queryData,
              a01Id:this.testA01Id
            }
            let removeRecords = this.removeTreeData;
    ...
    //if(this.state!=null&&this.state=="1"){
              ...
              if(res.code=='200'){
                if(this.state!=null&&this.state=="1"){
                  this.$confirm('保存成功, 是否生成数字化任务?', '提示', {
                    ...
                    createTaskByA01Id({a01Id:this.testA01Id}).then(ress => {
                      if(res.code=='200'){
                        this.state="0";
                        let taskId=ress.data
                        this.$confirm('生成成功, 是否进入数字化页面?', '提示', {
                          confirmButtonText: '确定',
                          cancelButtonText: '取消',
                          type: 'warning'
                        }).then(response => {
      ...
      getQueryData(params) {
        ...
        queryE01z1TreeGrid({a01Id:this.testA01Id,isClassIfy:"1"}).then(response => {
