    ...
    >
      ...
      <el-tab-pane label="待审批" name="pendapproval">
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row :gutter="10" ref="searchRow"  style="margin:0 0 0 0;padding-left: 0px">
            <el-form
              v-hasPermi="['checkBorrowApprove:browse:search']"
              v-if="showBorrowing"
              ref="borrowingParams"
              label-width="110px"
              class="search-form"
...
<!--                  :key="index"-->
<!--                  v-model="BorrowingColumn[index].isCol"-->
<!--                >-->
...
<!--            </el-col>-->
          ...
          <div class="btn">
            <el-button type="primary" size="mini" icon="el-icon-edit-outline" v-hasPermi="['checkBorrowApprove:browse:approve']" @click="approvalProcess">审批处理</el-button>

            <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['checkBorrowApprove:browse:export']" @click="handleExportBorrowing">导出
            </el-button>
          </div>
          ...
          >
            ...
            <el-table-column :resizable="false"  align="center" fixed="right" label="操作" width="100">
              <template slot-scope="scope">
                <el-button v-hasPermi="['checkBorrowApprove:browse:look']" type="text" size="mini" @click="viewDetals(scope.row)">查看详情</el-button>
...
<!--            </el-col>-->
          ...
          <div class="btn">
            <el-button type="primary" icon="el-icon-download" v-hasPermi="['checkBorrowApprove:browse:export']" size="mini" @click="handleExportBorrowing">导出
            </el-button>
          </div>
          <el-table
            :default-sort="{ prop: 'date', order: 'descending' }"
          ...
          >
            ...
            <el-table-column :resizable="false" align="center" fixed="right" label="操作" width="100">
              <template slot-scope="scope">
                ...
                <el-button type="text" size="mini" v-hasPermi="['checkBorrowApprove:browse:look']" @click="adoptViewDetals(scope.row)" >查看详情</el-button>
                <!--                            <el-button type="text" size="mini" @click="borrowingReturn(scope.row)">归还</el-button>-->
                <!--                            <el-button type="text" size="mini" @click="borrowingFeedback(scope.row)">反馈</el-button>-->
              </template>
            </el-table-column>
...
<!--            </el-col>-->
          ...
          <div class="btn">
            <el-button type="primary" icon="el-icon-download" size="mini" v-hasPermi="['checkBorrowApprove:browse:export']" @click="handleExportBorrowing">导出
            </el-button>
          </div>
          <el-table
            :default-sort="{ prop: 'date', order: 'descending' }"
          ...
          >
            ...
            <el-table-column :resizable="false" align="center" fixed="right" label="操作" width="100">
              <template slot-scope="scope">
                <!--                @click="viewDetals(scope.row)"-->
                <el-button type="text" v-hasPermi="['checkBorrowApprove:browse:look']" size="mini" @click="adoptViewDetals(scope.row)" >查看详情</el-button>
                <!--                            <el-button type="text" size="mini" @click="borrowingReturn(scope.row)">归还</el-button>-->
                <!--                            <el-button type="text" size="mini" @click="borrowingFeedback(scope.row)">反馈</el-button>-->
              </template>
            </el-table-column>
          ...
          <el-dialog title="借阅档案到期续借"  :append-to-body="true"  :visible.sync="borrowingFormVisible">
    ...
    <el-dialog title="查阅详情" :append-to-body="true"  :close-on-click-modal="false"  width="90%" class=" dialog-style dialog-basic" :visible.sync="viewDetalsVasable">
      ...
      <!--      class="tree"-->
      <div class="t-ctn"  style="width:100%; ">
    ...
    <!--  批量审批处理  -->
    <el-dialog title="审批处理" :append-to-body="true"  width="50%" :close-on-click-modal="false" class=" dialog-style dialog-basic" :visible.sync="viewDetalsVasable2">
      <div class="form-basic">审批信息</div>
      <el-row >
    ...
    <!--  已通过查看详情-->
    <el-dialog title="查阅详情" :append-to-body="true"   width="90%" :close-on-click-modal="false" class=" dialog-style dialog-basic" :visible.sync="viewDetalsVasable3">
...
<script>
  import {
    // getUserInfoAndArchivesRecords,
    // saveArchiveUsing,
    // getArchivesRecords,
    // quertA01ByName,
    // saveArchiveRenewAccess,
    conditionSearchArchives,
    // saveBorrowArchiveRenew,
    // saveArchivesCheckFeedBack,
    // giveBackArchives,
    queryE01zcRelaTreeByE01zcId
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 申请人列名
        BorrowingColumn: [
          { label: 'id',  isCol: false,width:100, prop: 'id' },
          { label: '隶属全宗', isCol: false,width:100, prop: 'orgId' },
          { label: '单位名称', isCol: false, width:100,prop: 'e01z824a' },
          { label: '人员信息id', isCol: false,width:100, prop: 'userId' },
    ...
    mounted() {
      ...
      setTimeout(()=>{
        ...
        console.log(this.searchRowHeight);
    ...
    methods: {
      ...
      getDefaultCheck(tree){
        if(tree.init){
          //选中选中的
          console.log(this.$refs[tree.id][0].getCheckedNodes(false,false));
          return ;
        }
        ...
        if(this.electronicCheck.length==0){
          this.$confirm('是否导出搜索后的审批数据?', "提示", {
            ...
            if(this.$route.params.sign==null){
              ...
              // console.log(s);
              // s = encodeURI(s);
              // downLoad("/export/excel/export?status=6&exportData=" + s)
            }
          })
      ...
      //       console.log(this.a0117s);
      //       console.log(row[col.property]);
      //       console.log(row);
      //       let returnValues='';
      ...
      //           return
      //       }
      //       if (row[column.property] == '0101'){
      //           return "国家级正职";
      //       }else if (row[column.property]=='0102'){
      //           return "国家级副职";
      //       }else if (row[column.property]=='0111'){
      //           return "省部";
      //       }else if (row[column.property]=='0112'){
      //           return "副省部";
      //       }else if (row[column.property]=='0121'){
      //           return "正局";
      //       }else if (row[column.property]=='0122'){
      //           return "副局";
      //       }else if (row[column.property]=='0131'){
      //           return "正县处";
      //       }else if (row[column.property]=='0132'){
      //           return "副县处";
      //       }else if (row[column.property]=='0141'){
      //           return "正乡科";
      //       }else if (row[column.property]=='0142'){
      //           return "副乡科";
      //       }else if (row[column.property]=='0150'){
      //           return "科员";
      //       }else if (row[column.property]=='0160'){
      //           return "办事员";
      //       }else if (row[column.property]=='0199'){
      //           return "未定职公务员";
      //       }else if (row[column.property]=='0211'){
      //           return "总理级";
      //       }else if (row[column.property]=='0212'){
      //           return "副总理级";
      //       }else if (row[column.property]=='0221'){
      //           return "正市（部）级";
      //       }else if (row[column.property]=='0222'){
      //           return "副市（部）级";
      //       }else if (row[column.property]=='0231'){
      //           return "正局级";
      //       }else if (row[column.property]=='0232'){
      //           return "副局级";
      //       }else if (row[column.property]=='0241'){
      //           return "正处级";
      //       }else if (row[column.property]=='0242'){
      //           return "副处级";
      //       }else if (row[column.property]=='0251'){
      //           return "正科级";
      //       }else if (row[column.property]=='0252'){
      //           return "副科级";
      //       }
      //   },
      //
      //   //政治面貌
      //   formatZZMM(row, column){
      //       if (row[column.property] === '') {
      //           return
      //       }
      //       if (row[column.property] == null){
      //           return
      //       }
      //       if (row[column.property] == '01'){
      //           return "中共党员";
      //       }else if (row[column.property]=='02'){
      //           return "预备党员";
      //       }else if (row[column.property]=='03'){
      //           return "共青团员";
      //       }else if (row[column.property]=='04'){
      //           return "民革";
      //       }else if (row[column.property]=='05'){
      //           return "民盟";
      //       }else if (row[column.property]=='06'){
      //           return "民建";
      //       }else if (row[column.property]=='07'){
      //           return "民进";
      //       }else if (row[column.property]=='08'){
      //           return "农工";
      //       }else if (row[column.property]=='09'){
      //           return "致公";
      //       }else if (row[column.property]=='10'){
      //           return "九三";
      //       }else if (row[column.property]=='11'){
      //           return "台盟";
      //       }else if (row[column.property]=='12'){
      //           return "无党派";
      //       }else if (row[column.property]=='13'){
      //           return "群众";
      //       }else if (row[column.property]=='14'){
      //           return "少先队员";
      //       }else {
      ...
      //       }else if (row[column.property] == '11'){
      //           return "查阅、摘抄";
      //       }else if (row[column.property] == '101'){
      //           return "查阅、打印";
      //       }else if (row[column.property] == '111'){
      //           return "查阅、摘抄、打印";
      //       }else if (row[column.property] == '2'){
      //           return "查阅";
      //       }else if (row[column.property] == '22'){
      //           return "查阅、摘抄";
      //       }else if (row[column.property] == '202'){
      //           return "查阅、复印";
      //       }else if (row[column.property] == '222'){
      //           return "查阅、摘抄、复印";
      //       }else if (row[column.property] == '3'){
      //           return "借阅";
      //       }else {
...
<style lang="scss" scoped>
  .t-ctn {
    width: 100%;
    //overflow-x: auto;  // ljh 注释掉  ie中会出现双滚动条
  }
