  ...
  <div class="app-borrowing">
    ...
    >
      ...
      <el-tab-pane label="待审批" name="pendapproval">
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row :gutter="10" ref="searchRow1" style="margin:0 0 0 0;padding-left: 0px">
            <el-form
              ...
              :inline="true"
              v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:search']"
            >
              ...
              <el-form-item label="申请借阅时间起" prop="startTime">
                <el-date-picker size="mini" v-model="borrowingParams0.startTime" type="date" placeholder="请选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item label="申请借阅时间至" prop="endTime">
                <el-date-picker size="mini" v-model="borrowingParams0.endTime" type="date" placeholder="请选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item>
          ...
          <div class="btn">
            <el-button type="primary" size="mini" icon="el-icon-edit-outline" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:approve']" @click="approvalProcessing">审批处理
            </el-button>

            <el-button type="primary" size="mini" icon="el-icon-download" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:export']" @click="handleExportBorrowing">导出
            </el-button>
          </div>
          <el-table
          ...
          >
            ...
            <el-table-column label="操作" align="center" width="100" fixed="right">
              <template slot-scope="scope">
                <el-button type="text" size="small" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:look']" @click="electronicFeedback(scope.row)">借看详情</el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <!-- 详情对话框 -->
        <el-dialog title="实体档案借阅申请详情" :append-to-body="true" :close-on-click-modal="false" :visible.sync="dialogDetailsVisible" width="85%" class="dialog-style">
          <el-table
            size="mini"
            :data="detailsData"
            :default-sort="{ prop: 'date', order: 'descending' }"
          ...
          >
            ...
            <el-table-column align="center" label="借档内容" show-overflow-tooltip prop="e01z111a"></el-table-column>
            <el-table-column align="center" label="档案类型" prop="ArchiveBorrowsType"></el-table-column>
            <el-table-column align="center" label="审批状态" show-overflow-tooltip prop="flag"
      ...
      <el-tab-pane label="已通过" name="passed">
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row :gutter="10" ref="searchRow2" style="margin:0 0 0 0;padding-left: 0px">
            ...
            >
              ...
              <el-form-item label="申请借阅时间起" prop="startTime">
                <el-date-picker size="mini" v-model="borrowingParams1.startTime" type="date" placeholder="选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item label="申请借阅时间至" prop="endTime">
                <el-date-picker size="mini" v-model="borrowingParams1.endTime" type="date" placeholder="请选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQueryBorrowing">搜索
          ...
          <div class="btn">
            <el-button type="primary" size="mini" icon="el-icon-plus" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:add']" @click="handleAddBorrowing">添加借阅记录</el-button>

            <el-button type="primary" size="mini" icon="el-icon-notebook-2" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:file']" @click="getReceipt">附件管理</el-button>

            <el-button type="primary" size="mini" icon="el-icon-s-help" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:rever']" @click="revertArchive">档案归还</el-button>

            <el-button type="primary" size="mini" icon="el-icon-receiving" v-hasPermi="['checkBorrowApprove:physicalBorrowApproval:renew']" @click="renewalOfMaturity">到期续借</el-button>
          ...
          <!-- 附件管理 -->
          <el-dialog title="附件管理" :append-to-body="true" :close-on-click-modal="false" width="700px" height="700px" :visible.sync="dialogVisible2" class="dialog-style">
            <el-upload
              ref="upload"
              name="part"
              multiple
              ...
              <el-button style="margin-left: 10px;" size="mini" type="success" @click="submitUpload"><i
                class="el-icon-upload el-icon--right"/>上传文件
              </el-button>
              <el-button size="mini" type="primary" @click="deleReceipt()"><i class="el-icon-error"
                                                                              style="margin-right:5px"/>删除文件
              </el-button>
            </el-upload>
            <div>
              <div style="float: left; margin-left: 5px;" slot="tip" class="el-upload-list__item-name">{{fileName}}
              </div>
        ...
        <!--  到期续借  -->
        <el-dialog title="到期续借"  :append-to-body="true" width="28%" :close-on-click-modal="false" class=" dialog-style dialog-basic" :visible.sync="viewDetalsVasable1">
          <div class="dialog-state">
            <span class="state">填写续借截止日期</span>
            <el-form ref="form" :model="approvalForm" style="width:90%;margin-top:11%;margin-left: 5%"
                     label-width="120px">
              <el-form-item prop="oldEndTime" label="借阅截止日期至">
                <el-date-picker size="mini" style="width:100%" v-model="approvalForm.oldEndTime" type="date"
                                placeholder="请选择日期"/>
              </el-form-item>
              <el-form-item prop="endTime" label="续借截止日期至">
                <el-date-picker size="mini" style="width:100%" v-model="approvalForm.endTime" type="date"
                                placeholder="请选择日期"/>
              </el-form-item>
            </el-form>
          </div>
        ...
        <el-col :span="24" :xs="24" style="padding: 10px;margin: 0;height: calc(100vh - 86px)">
          <el-row :gutter="10" ref="searchRow3" style="margin:0 0 0 0;padding-left: 0px">
            ...
            >
              ...
              <el-form-item label="申请借阅时间起" prop="startTime">
                <el-date-picker size="mini" v-model="borrowingParams2.startTime" type="date" placeholder="选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item label="申请借阅时间至" prop="endTime">
                <el-date-picker size="mini" v-model="borrowingParams2.endTime" type="date" placeholder="请选择时间"
                                style="width: 150px"/>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQueryBorrowing">搜索
          ...
          >
            <el-table-column type="selection" width="40"/>
            <el-table-column style="width: 20px" type="index" show-overflow-tooltip align="center" label="序号" :index="customIndex2"/>
            <el-table-column
              align="center"
              show-overflow-tooltip
              :resizable="false"
    ...
    <el-col v-if="formShow" style="margin: 0;padding: 0;border: 1px solid #cccccc;height: calc(100vh - 86px);overflow: auto;" :span="24" :xs="24"
            >
      <el-row style="padding-left: 10%;padding-right: 10%">
        ...
        <el-col :span="24">
          ...
          <el-form ref="addForm" :model="addForm" label-width="150px" :rules="rules"
                   style="width:90%;margin-top:25px;padding-left: 2%;padding-right: 2%">
            ...
            <el-col :span="8">
              <el-form-item label="申请借阅起始日期" prop="startTime">
                <el-date-picker size="mini" v-model="addForm.startTime" type="date" placeholder="选择申请开始时间"
                                style="width: 100%;"/>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="申请借阅截止日期至" style="text-align:right;" prop="endTime">
                <el-date-picker size="mini" v-model="addForm.endTime" type="date" placeholder="选择申请截止时间至"
                                style="width: 100%;"/>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="8">
              <el-form-item label="打印内容" prop="printContents">
                <el-input v-model="addForm.printContents" size="mini" type="textarea" placeholder="若打印请输入打印内容"
                          style="width: 100%;"/>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="备注" prop="e01z831">
                <el-input v-model="addForm.e01z831" size="mini" placeholder="请输入备注" style="width: 100%;"/>
              </el-form-item>
            </el-col>
            <el-col :span="16">
            ...
            <el-col :span="24">
              <el-form-item label="档案归还状态"   prop="archiveStatus">
                <el-radio-group style="width: 200px"  v-model="addForm.archiveStatus">
        ...
        <el-col :span="24" style="padding: 10px">
          <div style="width:100%;margin: 0 auto">
            <el-button style="float: right;margin-right: 10px;width:80px" size="mini" @click="resetAddForm">重置表单
            </el-button>
          </div>
        </el-col>
        <el-col :span="24">
          <div
            style="width: 100%;margin-bottom:15px;line-height:30px;border-bottom: 1px solid #ccc;margin: 0 auto;padding:20px 0 0 0">
            第二步：请选择需要借阅的干部档案
          </div>
          ...
          <el-row :gutter="10"   style="margin: 0 auto;padding:25px 0 0 0">
            <el-form v-model="formInline"  :inline="true" :model="formInline" label-width="80px">
              ...
              <el-form-item label="干部状态" style="margin-left: 20px">
                <el-select v-model="a01state" style="width: 150px" filterable class="inputW" placeholder="请选择干部状态"
                           size="mini">
              ...
              <el-form-item style="margin-left: 20px" prop="deptName">
                <el-input v-model="formInline.deptName" @keyup.enter.native="search(1)" size="mini" style="width: 150px"
                          placeholder="请输入部门名称"/>
              </el-form-item>
              <el-form-item>
              ...
              <el-form-item style="margin-left: 20px" prop="a0101">
                <el-input v-model="formInline.a0101" @keyup.enter.native="search(2)" size="mini" style="width: 150px"
                          placeholder="请输入干部名称"/>
              </el-form-item>
              <el-form-item>
          ...
          <el-col :span="24" style="padding-left: 2%;padding-right: 2%">
            <el-table height="222" empty-text=" " tooltip-effect="light" size="mini" ref="searchTableData1" class="table-style"
                      :data="searchTableData" border @cell-dblclick="deptClick" @selection-change="gridSelectionChange"
...
<!--              <el-table-column prop="number" key="6" v-if="departOrPerson==2" label="干部编号" align="center"/>-->
              ...
              <el-table-column show-overflow-tooltip prop="a0104" key="8" label="性别" v-if="departOrPerson==2" align="center"
                               :formatter="getFullName"/>
              <el-table-column show-overflow-tooltip prop="a0107" key="9" label="出生日期" v-if="departOrPerson==2" align="center"
                               :formatter="getFullName"/>
              <el-table-column show-overflow-tooltip prop="a0117" key="10" label="民族" v-if="departOrPerson==2" align="center"
                               :formatter="getFullName"/>
              <el-table-column show-overflow-tooltip prop="a0141" key="11" label="政治面貌" v-if="departOrPerson==2" align="center"
                               :formatter="getFullName"/>
            </el-table>
          </el-col>
        </el-col>
        ...
        <el-col :span="24" style="padding: 10px">
          <div style="width:100%;margin: 0 auto">
            <el-button type="primary" style="float: right;margin-right: 10px;width:80px" size="mini" @click="typeClick">
              添加
            </el-button>
          </div>
        </el-col>
...
<!--          <el-table-column align="center" v-if="true" label="编号" prop="number"/>-->
          <el-table-column align="center" show-overflow-tooltip v-if="true" label="单位名称" prop="deptName"/>
          <el-table-column align="center"  v-if="true" label="职务" prop="duty" :formatter="getFullName"/>
          <el-table-column align="center" show-overflow-tooltip v-if="true" label="借档内容" prop="e01z111a"/>
        ...
        <el-button style="float: right;margin: 10px;" size="mini" type="primary" @click="onGrid('addForm')">添加记录
        </el-button>
    ...
    <!--  批量审批处理  -->
    <el-dialog title="审批处理"  :append-to-body="true"  width="60%" :close-on-click-modal="false" class=" dialog-style dialog-basic" :visible.sync="approvalProcessingShow">
      <div class="form-basic">审批信息</div>
      <el-form ref="approvalUpdateForm" :rules="rules" :model="approvalUpdateForm" style="margin-left: 10%;margin-right: 10%" :inline="true"
               label-width="120px">
    ...
    <!-- 借看详情-->
    <el-dialog title="实体档案借阅详情" :append-to-body="true"  :close-on-click-modal="false" width="90%" class=" dialog-style dialog-basic" :visible.sync="dialogDetailsVisible1">

      <div class="form-basic" v-if="applyStates=='0'">审批信息</div>
      <el-form ref="approvalUpdateForm" v-if="applyStates=='0'" :rules="rules" :model="approvalForm1" style="padding:7px 0 0 0" :inline="true"
               label-width="120px">
        <el-form-item prop="e01z801" label="审批人">
      ...
      <div style="padding:3px 0 1px 0">
        <el-col style="margin-left: 2%" :span="1.5">
          <el-button type="primary" icon="el-icon-download" @click="exportx" size="mini">导出</el-button>
        </el-col>
      </div>
      <el-table :data="viewDetailDatas"
        ...
        <el-table-column prop="startTime" label="申请借阅起始日期" align="center" :formatter="getFullName"/>
        <el-table-column prop="endTime" label="申请借阅截止日期至" align="center" :formatter="getFullName"/>
      </el-table>
    ...
    data() {
      return {
        ...
        pendingApprovalCol: [
          ...
          {label: '申请借阅截止日期至', prop: 'endTime', isCol: true, width: "150px", formatter: this.getFullName},
        ...
        approvalForm: {},
        //审批form
        approvalUpdateForm: {e01z801: "",e01z817:"",flag:""},
        //借看详情审批
        approvalForm1: {
          e01z801:""
        },
    ...
    methods: {
      ...
      renewalOfMaturity() {
        if (this.electronicCheck.length != 1) {
          ...
          if (this.electronicCheck[0].applyStatus == "到期续借") {
            this.$message({
              message: "该档案已续借过，请重新申请",
              type:'warning'
            })
          } else {
      ...
      archivesaFunction(all) {
        if(this.applyStates=="0"||all){
          ...
          console.info(param.searchValue)
          getEntityArchiveAApplyRecord(param).then(response =>{
      ...
      revertArchive() {
        if (this.electronicCheck.length > 0) {
          let ids = "";
          for (let n in this.electronicCheck) {
            if (this.electronicCheck[n].archiveStatus != "已归还") {
              ids = ids +","+ this.electronicCheck[n].id
            }
          }
          if (ids == "") {
            this.$message({
            ...
            revertArchives(param).then(response => {
              if (response.code == 200) {
                this.$message({
                  type: "success",
                  message: "归还成功"
      ...
      handleExportBorrowing() {
        ...
        // console.log(s);
        // s = encodeURI(s);
        // downLoad("/export/excel/export?status=9&exportData=" + s)
