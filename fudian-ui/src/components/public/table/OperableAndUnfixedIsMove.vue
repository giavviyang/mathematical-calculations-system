        ...
        <el-table-column align="center" :label="operableLable" min-width="100" max-width="200">
          <template slot-scope="scope">
            <div v-for="item in operable">
              <el-button
                type="text"
                v-if="item.operableName==='上移'"
                :disabled="scope.$index==0"
                @click="onlinePreview(scope.row,scope.$index)"
                size="mini">{{ item.operableName }}
              </el-button>
              <el-button
                type="text"
                v-if="item.operableName==='下移'"
                :disabled="scope.$index+1 == tableData.length"
                @click="onlinePreviewOne(scope.row,scope.$index)"
                size="mini">{{ item.operableName }}
              </el-button>
            </div>
          </template>
...
export default {
  ...
  methods: {
    ...
    onlinePreview(val, index) {
      this.$emit('onlinePreview', val, index);
    },
    onlinePreviewOne(val, index) {
      this.$emit('onlinePreviewOne', val, index);
    },
